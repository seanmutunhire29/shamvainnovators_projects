...
for disease, info in DISEASE_PROFILES.items():
    ...
    for symptom in info["symptoms"]:
        if st.checkbox(symptom, key=f"elisiya_{disease}_{symptom}"):
...
if st.button("Analyze Health Profile"):
    ...
    st.session_state["elisiya_diseases_to_monitor"] = diseases_to_monitor
...
for index, step in enumerate(guidelines):
    st.checkbox(step, key=f"elisiya_monitor_{selected_monitor_disease}_{index}")
